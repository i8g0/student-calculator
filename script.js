...
// ===== FEATURE 25: ABSENCE STATISTICS =====
function updateAbsenceStatistics() {
    const statsSection = document.getElementById('absence-statistics');
    const statsContent = document.getElementById('absence-stats-content');
    
    if (!statsSection || !statsContent || !absenceCourses || absenceCourses.length === 0) {
        if (statsSection) statsSection.style.display = 'none';
        return;
    }
    
    // Calculate overall statistics
    let totalCourses = absenceCourses.length;
    let totalAbsenceHours = 0;
    let totalHours = 0;
    let coursesAtRisk = 0;
    let coursesSafe = 0;
    let highestAbsence = 0;
    let lowestAbsence = 100;
    
    absenceCourses.forEach(course => {
        const stats = calculateCourseAbsenceStats(course);
        totalAbsenceHours += stats.absenceHours;
        ...
        if (stats.absencePercentage > highestAbsence) {
            highestAbsence = stats.absencePercentage;
        }
        if (stats.absencePercentage < lowestAbsence) {
            lowestAbsence = stats.absencePercentage;
        }
    });
    
    const overallPercentage = totalHours > 0 ? (totalAbsenceHours / totalHours) * 100 : 0;
    
    statsContent.innerHTML = `
        <div class="stat-card" style="padding: 1rem; background: linear-gradient(135deg, #667eea15 0%, #764ba215 100%); border-radius: 8px; text-align: center;">
            <div class="stat-value" style="font-size: 2rem; font-weight: bold; color: var(--primary-color, #667eea);">${totalCourses}</div>
            <div class="stat-label" style="color: var(--text-secondary, #666); margin-top: 0.5rem;">إجمالي المواد</div>
        </div>
        <div class="stat-card" style="padding: 1rem; background: linear-gradient(135deg, #f093fb15 0%, #f5576c15 100%); border-radius: 8px; text-align: center;">
            <div class="stat-value" style="font-size: 2rem; font-weight: bold; color: #f5576c;">${overallPercentage.toFixed(2)}%</div>
            <div class="stat-label" style="color: var(--text-secondary, #666); margin-top: 0.5rem;">متوسط نسبة الغياب</div>
        </div>
        <div class="stat-card" style="padding: 1rem; background: linear-gradient(135deg, #4facfe15 0%, #00f2fe15 100%); border-radius: 8px; text-align: center;">
            <div class="stat-value" style="font-size: 2rem; font-weight: bold; color: #4facfe;">${totalAbsenceHours.toFixed(1)}</div>
            <div class="stat-label" style="color: var(--text-secondary, #666); margin-top: 0.5rem;">ساعات الغياب الإجمالية</div>
        </div>
        <div class="stat-card" style="padding: 1rem; background: linear-gradient(135deg, #43e97b15 0%, #38f9d715 100%); border-radius: 8px; text-align: center;">
            <div class="stat-value" style="font-size: 2rem; font-weight: bold; color: #43e97b;">${coursesSafe}</div>
            <div class="stat-label" style="color: var(--text-secondary, #666); margin-top: 0.5rem;">مواد آمنة</div>
        </div>
        <div class="stat-card" style="padding: 1rem; background: linear-gradient(135deg, #fa709a15 0%, #fee14015 100%); border-radius: 8px; text-align: center;">
            <div class="stat-value" style="font-size: 2rem; font-weight: bold; color: #fa709a;">${coursesAtRisk}</div>
            <div class="stat-label" style="color: var(--text-secondary, #666); margin-top: 0.5rem;">مواد معرضة للخطر</div>
        </div>
        <div class="stat-card" style="padding: 1rem; background: linear-gradient(135deg, #30cfd015 0%, #33086715 100%); border-radius: 8px; text-align: center;">
            <div class="stat-value" style="font-size: 2rem; font-weight: bold; color: #30cfd0;">${highestAbsence.toFixed(2)}%</div>
            <div class="stat-label" style="color: var(--text-secondary, #666); margin-top: 0.5rem;">أعلى نسبة غياب</div>
        </div>
    `;
...
function calculateCourseAbsenceStats(course) {
    let absenceHours = 0;
    let totalHours = 0;
    
    [course.lectures, course.exercises, course.labs].forEach(type => {
        if (type && type.sessions) {
            type.sessions.forEach(session => {
                const sessionHours = session.hours || 1.5;
                const sessionTotal = sessionHours * course.weeks;
                totalHours += sessionTotal;
                absenceHours += (session.absenceCount || 0) * sessionHours;
            });
        }
    });
    
    const percentage = totalHours > 0 ? (absenceHours / totalHours) * 100 : 0;
...
// ===== FEATURE 26: ABSENCE GOALS =====
let absenceGoals = JSON.parse(localStorage.getItem('absenceGoals')) || [];

function initAbsenceGoals() {
    const addGoalBtn = document.getElementById('add-goal-btn');
    if (addGoalBtn) {
        addGoalBtn.addEventListener('click', showAddGoalModal);
    }
    updateAbsenceGoalsDisplay();
}

function showAddGoalModal() {
    const courseName = prompt('أدخل اسم المادة:');
    if (!courseName) return;
    
    const maxPercentage = prompt('أدخل الحد الأقصى المطلوب لنسبة الغياب (مثال: 15):');
    ...
    absenceGoals.push(goal);
    localStorage.setItem('absenceGoals', JSON.stringify(absenceGoals));
    updateAbsenceGoalsDisplay();
    showToast('تم إضافة الهدف بنجاح! 🎯', 'success');
}

function updateAbsenceGoalsDisplay() {
    const goalsSection = document.getElementById('absence-goals');
    const goalsContent = document.getElementById('absence-goals-content');
    
    if (!goalsSection || !goalsContent) return;
    
    if (absenceGoals.length === 0) {
        goalsSection.style.display = 'none';
        return;
    }
    
    goalsContent.innerHTML = absenceGoals.map(goal => {
        const course = absenceCourses.find(c => c.name === goal.courseName);
        const currentPercentage = course ? calculateCourseAbsenceStats(course).absencePercentage : 0;
        const isAchieved = currentPercentage <= goal.maxPercentage;
        const progress = Math.min((currentPercentage / goal.maxPercentage) * 100, 100);
        
        return `
            <div class="goal-card" style="padding: 1rem; margin-bottom: 1rem; background: ${isAchieved ? 'linear-gradient(135deg, #43e97b15 0%, #38f9d715 100%)' : 'linear-gradient(135deg, #fa709a15 0%, #fee14015 100%)'}; border-radius: 8px; border: 2px solid ${isAchieved ? '#43e97b' : '#fa709a'};">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                    <strong style="color: var(--text-primary, #333);">${goal.courseName}</strong>
                    <button onclick="deleteGoal(${goal.id})" style="background: none; border: none; color: #f5576c; cursor: pointer; padding: 0.25rem;">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div style="margin-bottom: 0.5rem;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.25rem;">
                        <span style="font-size: 0.9rem; color: var(--text-secondary, #666);">الهدف: ${goal.maxPercentage}%</span>
                        <span style="font-size: 0.9rem; color: var(--text-secondary, #666);">الحالي: ${currentPercentage.toFixed(2)}%</span>
                    </div>
                    <div style="height: 8px; background: #e0e0e0; border-radius: 4px; overflow: hidden;">
                        <div style="height: 100%; width: ${progress}%; background: ${isAchieved ? '#43e97b' : '#fa709a'}; transition: width 0.3s;"></div>
                    </div>
                </div>
                <div style="text-align: center; color: ${isAchieved ? '#43e97b' : '#fa709a'}; font-weight: bold;">
                    ${isAchieved ? '✅ تم تحقيق الهدف!' : '⚠️ لم يتم تحقيق الهدف بعد'}
                </div>
            </div>
        `;
    }).join('');
...
function deleteGoal(goalId) {
    absenceGoals = absenceGoals.filter(g => g.id !== goalId);
    localStorage.setItem('absenceGoals', JSON.stringify(absenceGoals));
    updateAbsenceGoalsDisplay();
    showToast('تم حذف الهدف', 'success');
}

// ===== FEATURE 30: SMART RECOMMENDATIONS =====
function updateAbsenceRecommendations() {
    const recSection = document.getElementById('absence-recommendations');
    const recContent = document.getElementById('absence-recommendations-content');
    
    if (!recSection || !recContent || !absenceCourses || absenceCourses.length === 0) {
    ...
    recContent.innerHTML = recommendations.map(rec => `
        <div class="recommendation-item" style="padding: 1rem; margin-bottom: 1rem; background: white; border-radius: 8px; border-right: 4px solid ${rec.color || '#667eea'};">
            <div style="display: flex; align-items: start; gap: 1rem;">
                <div style="font-size: 1.5rem; color: ${rec.color || '#667eea'};">
                    <i class="${rec.icon || 'fas fa-lightbulb'}"></i>
                </div>
                <div style="flex: 1;">
                    <strong style="display: block; margin-bottom: 0.5rem; color: var(--text-primary, #333);">${rec.title}</strong>
                    <p style="margin: 0; color: var(--text-secondary, #666); font-size: 0.9rem;">${rec.message}</p>
                </div>
            </div>
        </div>
    `).join('');
...
function generateSmartRecommendations() {
    ...
    absenceCourses.forEach(course => {
        const stats = calculateCourseAbsenceStats(course);
        const percentage = stats.absencePercentage;
        
        if (percentage >= 25) {
            recommendations.push({
                title: `⚠️ تحذير: ${course.name}`,
                message: `نسبة غيابك في ${course.name} وصلت ${percentage.toFixed(2)}%. يجب عليك الحضور فوراً لتجنب الحرمان!`,
                color: '#f5576c',
                icon: 'fas fa-exclamation-triangle'
            });
        } else if (percentage >= 20) {
            recommendations.push({
                title: `🔴 خطر: ${course.name}`,
                message: `نسبة غيابك في ${course.name} قريبة من الحد المسموح (${percentage.toFixed(2)}%). حاول الحضور بانتظام.`,
                color: '#fa709a',
                icon: 'fas fa-exclamation-circle'
            });
        } else if (percentage >= 15) {
            recommendations.push({
                title: `🟡 انتباه: ${course.name}`,
                message: `نسبة غيابك في ${course.name} هي ${percentage.toFixed(2)}%. حاول تقليل الغياب قليلاً.`,
                color: '#fee140',
    ...
    const overallStats = absenceCourses.reduce((acc, course) => {
        const stats = calculateCourseAbsenceStats(course);
        acc.totalAbsence += stats.absenceHours;
        acc.totalHours += stats.totalHours;
        return acc;
    }, { totalAbsence: 0, totalHours: 0 });
    
    const overallPercentage = overallStats.totalHours > 0 ? (overallStats.totalAbsence / overallStats.totalHours) * 100 : 0;
    
    if (overallPercentage < 10) {
        recommendations.push({
            title: '✅ ممتاز!',
            message: 'معدل غيابك الإجمالي ممتاز. استمر في الحضور بانتظام.',
            color: '#43e97b',
    ...
    if (recommendations.length === 0) {
        recommendations.push({
            title: '👍 أداء جيد',
            message: 'معدل غيابك في جميع المواد جيد. استمر في الحضور بانتظام.',
            color: '#4facfe',
...
// Hook into loadCourses to update statistics
const originalLoadCourses = loadCourses;
loadCourses = function() {
    if (originalLoadCourses) originalLoadCourses();
    setTimeout(() => {
        updateAbsenceStatistics();
        updateAbsenceGoalsDisplay();
        updateAbsenceRecommendations();
    }, 100);
};

// Initialize goals when DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        initAbsenceGoals();
        updateAbsenceStatistics();
        updateAbsenceRecommendations();
        initSemesterNotifications();
        initTouchGestures();
...
// ===== FEATURE 81: SEMESTER NOTIFICATIONS =====
function initSemesterNotifications() {
    // Check for semester dates
    const semesterDates = JSON.parse(localStorage.getItem('semesterDates')) || {
        startDate: null,
        endDate: null,
        examStartDate: null
    };
    
    // If no dates set, prompt user (optional)
    if (!semesterDates.startDate) {
        // User can set dates manually later
        return;
    }
    
    checkSemesterDates(semesterDates);
...
function checkSemesterDates(dates) {
    const now = new Date();
    ...
    if (startDate && now >= startDate && now <= new Date(startDate.getTime() + 7 * 24 * 60 * 60 * 1000)) {
        showToast('🎓 بداية الفصل الدراسي! حظاً موفقاً!', 'info');
    }
    
    if (endDate && now >= new Date(endDate.getTime() - 7 * 24 * 60 * 60 * 1000) && now <= endDate) {
        showToast('📚 نهاية الفصل الدراسي قريبة! استعد للامتحانات', 'warning');
    }
    
    if (examStart && now >= new Date(examStart.getTime() - 3 * 24 * 60 * 60 * 1000) && now <= examStart) {
        showToast('📝 موعد الامتحانات قريب! ابدأ بالمراجعة', 'warning');
    }
}

// Function to set semester dates (can be called from UI)
function setSemesterDates(startDate, endDate, examStartDate) {
    const dates = {
        startDate: startDate,
        endDate: endDate,
    ...
    showToast('تم حفظ تواريخ الفصل الدراسي', 'success');
}

// ===== FEATURE 95 & 96: CLOUD SYNC & LOCAL STORAGE ENHANCEMENT =====
...
function loadDataFromLocalStorage(key) {
    try {
        ...
        // Try backup
        try {
            const backupStr = localStorage.getItem(`${key}_backup`);
            if (backupStr) {
...
function enhanceDataPersistence() {
    // Auto-save every 30 seconds
    setInterval(() => {
        if (absenceCourses && absenceCourses.length > 0) {
            saveDataToLocalStorage('absenceCourses', absenceCourses);
        }
        if (newAbsenceCourses && newAbsenceCourses.length > 0) {
            saveDataToLocalStorage('newAbsenceCourses', newAbsenceCourses);
        }
        if (absenceGoals && absenceGoals.length > 0) {
            saveDataToLocalStorage('absenceGoals', absenceGoals);
        }
    }, 30000);
}

// Initialize data persistence
enhanceDataPersistence();

// ===== FEATURE 98: TOUCH GESTURES =====
function initTouchGestures() {
    ...
    document.addEventListener('touchend', (e) => {
        ...
        // Swipe right (previous tab) - only on mobile
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > minSwipeDistance) {
            if (deltaX > 0 && window.innerWidth <= 768) {
                // Swipe right - go to previous tab
                navigateToPreviousTab();
            } else if (deltaX < 0 && window.innerWidth <= 768) {
...
function navigateToPreviousTab() {
    const tabs = ['rate', 'absence', 'science-college', 'applied-college', 'cumulative', 'semester', 'analytics'];
...
// ===== FEATURE 100: PERFORMANCE OPTIMIZATIONS =====
    ...
    const tips = [
        {
            icon: 'fas fa-clock',
            title: 'إدارة الوقت',
            description: 'خصص وقتاً محدداً للدراسة يومياً. استخدم تقنية البومودورو: 25 دقيقة دراسة + 5 دقائق راحة.',
            color: '#667eea'
        },
        {
            icon: 'fas fa-book',
            title: 'القراءة الفعالة',
            description: 'اقرأ النص مرة بسرعة للإطلاع، ثم اقرأه مرة أخرى بتركيز. استخدم طريقة SQ3R: Survey, Question, Read, Recite, Review.',
            color: '#f093fb'
        },
        {
            icon: 'fas fa-pen',
            title: 'التلخيص',
            description: 'لخص المادة بأسلوبك الخاص. الكتابة تساعد على التذكر والاستيعاب. استخدم الخرائط الذهنية.',
            color: '#4facfe'
        },
        {
            icon: 'fas fa-repeat',
            title: 'التكرار المتباعد',
            description: 'راجع المادة بعد يوم، ثم بعد أسبوع، ثم بعد شهر. التكرار المتباعد يساعد على التذكر طويل الأمد.',
            color: '#43e97b'
        },
        {
            icon: 'fas fa-users',
            title: 'الدراسة الجماعية',
            description: 'ادرس مع زملائك. شرح المادة للآخرين يساعدك على فهمها بشكل أعمق.',
            color: '#fa709a'
        },
        {
            icon: 'fas fa-bed',
            title: 'النوم الكافي',
            description: 'احصل على 7-9 ساعات نوم يومياً. النوم يساعد الدماغ على ترسيخ المعلومات.',
            color: '#fee140'
        },
        {
            icon: 'fas fa-dumbbell',
            title: 'الرياضة',
            description: 'مارس الرياضة بانتظام. الرياضة تحسن الذاكرة والتركيز.',
            color: '#30cfd0'
        },
        {
            icon: 'fas fa-mobile-alt',
            title: 'تقليل المشتتات',
            description: 'أغلق الإشعارات على هاتفك أثناء الدراسة. استخدم تطبيقات حظر المواقع المشتتة.',
            color: '#f5576c'
        }
    ];
    
    tipsContent.innerHTML = tips.map((tip, index) => `
        <div class="tip-card" style="padding: 1.5rem; margin-bottom: 1.5rem; background: linear-gradient(135deg, ${tip.color}15 0%, ${tip.color}05 100%); border-radius: 12px; border-right: 4px solid ${tip.color}; transition: transform 0.3s;">
            <div style="display: flex; align-items: start; gap: 1rem;">
                <div style="font-size: 2rem; color: ${tip.color}; flex-shrink: 0;">
                    <i class="${tip.icon}"></i>
                </div>
                <div style="flex: 1;">
                    <h4 style="margin: 0 0 0.5rem 0; color: var(--text-primary, #333); font-size: 1.2rem;">${tip.title}</h4>
                    <p style="margin: 0; color: var(--text-secondary, #666); line-height: 1.6;">${tip.description}</p>
                </div>
            </div>
        </div>
    `).join('');
...
// ===== FEATURE 108: EDUCATIONAL VIDEOS =====
function initEducationalVideos() {
    const videosContent = document.getElementById('educational-videos-content');
    ...
    videosContent.innerHTML = videos.map((video, index) => `
        <div class="video-card" style="margin-bottom: 2rem; padding: 1.5rem; background: var(--card-bg, #fff); border-radius: 12px; box-shadow: var(--shadow-md, 0 4px 6px rgba(0,0,0,0.1));">
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                <div style="font-size: 2rem; color: var(--primary-color, #667eea);">
                    <i class="${video.icon}"></i>
                </div>
                <div>
                    <h4 style="margin: 0 0 0.25rem 0; color: var(--text-primary, #333);">${video.title}</h4>
                    <p style="margin: 0; color: var(--text-secondary, #666); font-size: 0.9rem;">${video.description}</p>
                </div>
            </div>
    ...
    // Add placeholder message if no videos
    if (videos.length === 0) {
        videosContent.innerHTML = `
            <div style="text-align: center; padding: 3rem; color: var(--text-secondary, #666);">
                ...
                <p>سيتم إضافة الفيديوهات التعليمية قريباً</p>
            </div>
        `;
    }
}

// ===== FEATURE 111: AI PREDICTION =====
function predictPerformance(gpa, courses, absenceRate) {
    // Simple AI-like prediction based on historical patterns
    const predictions = {
        nextSemesterGPA: null,
        graduationGPA: null,
        recommendations: []
    };
    ...
    const avgCourseGrade = courses.reduce((sum, c) => sum + (c.grade || 0), 0) / courses.length;
    const predictedNext = (gpa * 0.7) + (avgCourseGrade * 0.3);
    predictions.nextSemesterGPA = Math.min(predictedNext.toFixed(2), 5.0);
    
    // Predict graduation GPA (assuming 8 semesters)
    const remainingSemesters = 8 - Math.floor((courses.length || 0) / 5);
    if (remainingSemesters > 0) {
        const predictedGraduation = (gpa * 0.6) + (predictedNext * 0.4);
        predictions.graduationGPA = Math.min(predictedGraduation.toFixed(2), 5.0);
    }
    ...
    if (gpa < 3.0) {
        predictions.recommendations.push({
            type: 'warning',
            message: 'معدلك التراكمي أقل من 3.0. ركز على رفع المعدل في الفصول القادمة.',
            action: 'ضع خطة دراسية محكمة'
        });
    }
    ...
    if (absenceRate > 15) {
        predictions.recommendations.push({
            type: 'danger',
            message: `نسبة غيابك ${absenceRate.toFixed(2)}% عالية. حاول الحضور بانتظام.`,
            action: 'ضع جدول حضور صارم'
        });
    }
    ...
    if (gpa >= 4.5) {
        predictions.recommendations.push({
            type: 'success',
            message: 'معدلك ممتاز! استمر في الأداء الجيد.',
            action: 'حافظ على نفس المستوى'
        });
    }
...
// ===== FEATURE 114: PERSONALIZED RECOMMENDATIONS =====
function generatePersonalizedRecommendations() {
    const recommendations = [];
    
    // Get user data
    const gpa = parseFloat(localStorage.getItem('lastGPA')) || 0;
    const calculations = JSON.parse(localStorage.getItem('calculations')) || [];
    const absenceStats = calculateOverallAbsenceStats();
    
    // Analyze performance trends
    if (calculations.length >= 2) {
        const recent = calculations.slice(-3);
        const trend = recent[recent.length - 1].result - recent[0].result;
        ...
        if (trend > 0) {
            recommendations.push({
                ...
                title: 'تحسين مستمر!',
                message: 'أداؤك في تحسن مستمر. استمر في نفس النهج.',
                color: '#43e97b'
            });
        } else if (trend < 0) {
            recommendations.push({
                ...
                title: 'انتبه للتراجع',
                message: 'لاحظنا تراجعاً في أدائك. راجع استراتيجيتك الدراسية.',
                color: '#fa709a'
            });
        }
    }
    ...
    if (gpa > 0) {
        if (gpa < 2.5) {
            recommendations.push({
                type: 'danger',
                icon: 'fas fa-exclamation-triangle',
                title: 'تحسين عاجل مطلوب',
                message: `معدلك الحالي ${gpa.toFixed(2)} يحتاج تحسين. ركز على المواد الأساسية وراجع مع المدرسين.`,
                color: '#f5576c'
            });
            ...
            recommendations.push({
                ...
                icon: 'fas fa-trophy',
                title: 'أداء ممتاز',
                message: `معدلك ${gpa.toFixed(2)} ممتاز! يمكنك التقدم للدراسات العليا أو المنح الدراسية.`,
                color: '#4facfe'
            });
        }
    }
    
    // Absence-based recommendations
    if (absenceStats.overallPercentage > 20) {
        recommendations.push({
            type: 'warning',
            icon: 'fas fa-calendar-times',
            title: 'مشكلة في الحضور',
            message: `نسبة غيابك ${absenceStats.overallPercentage.toFixed(2)}% عالية. الحضور ضروري للنجاح.`,
            color: '#fee140'
        });
    }
    ...
    if (lastStudyDate) {
        const daysSince = (Date.now() - new Date(lastStudyDate)) / (1000 * 60 * 60 * 24);
        if (daysSince > 3) {
            recommendations.push({
                type: 'info',
                icon: 'fas fa-book',
                title: 'وقت الدراسة',
                message: `مر ${Math.floor(daysSince)} أيام منذ آخر دراسة. جرب الدراسة بانتظام لتحسين النتائج.`,
                color: '#667eea'
            });
        }
    }
...
function calculateOverallAbsenceStats() {
    if (!absenceCourses || absenceCourses.length === 0) {
        return { overallPercentage: 0, totalHours: 0, absenceHours: 0 };
    }
    ...
    absenceCourses.forEach(course => {
        const stats = calculateCourseAbsenceStats(course);
        totalHours += stats.totalHours;
        absenceHours += stats.absenceHours;
    });
    
    return {
        overallPercentage: totalHours > 0 ? (absenceHours / totalHours) * 100 : 0,
        totalHours,
        absenceHours
    };
}

// Initialize study tips and videos when DOM is ready
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        initStudyTips();
        initEducationalVideos();
    });
} else {
    initStudyTips();
    initEducationalVideos();
